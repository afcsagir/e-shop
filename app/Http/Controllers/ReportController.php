...
class ReportController extends Controller
{
	public function showReportByDate ($time = null, Request $request) 
	{
		if (($time !== null) && ($time == 'today')) {
			$time = date("Y-m-d");
			$sales = SoldProduct::whereDate('updated_at', $time)->get();
			...
			$sales = SoldProduct::whereBetween('updated_at', [$time7, $time])->get();
			return view('report-by-date')->with('sales', $sales);

		} else if (($time !== null) && ($time == 'month')) {
			$time = date("m");
			$sales = SoldProduct::whereMonth('updated_at', $time)->get();
			return view('report-by-date')->with('sales', $sales);
			...
			$sales = SoldProduct::whereMonth('updated_at', $time7)->get();
			return view('report-by-date')->with('sales', $sales);
			...
			$sales = SoldProduct::whereYear('updated_at', $time7)->get();
			return view('report-by-date')->with('sales', $sales);
			...
			$sales = SoldProduct::whereBetween('updated_at', [$timeFrom, $timeTo])->get();
